...
# ---------------------------------------------------------------- patients
# A PATIENT IS SCORED WHERE IT LIES.  "Placing one triaged patient inside the
# correct destination zone +5"; -3 for one left outside it, -5 for one shoved
# into the WRONG zone.  Nothing asks for it to be lifted or stood up, so it is
# never ingested -- the nose plough catches it at 6.5 mm, below its centre of
# mass, and the robot drives it there.
#
# Being selective is not optional: a wrong zone is -5 against -3 for leaving it
# alone, so an indiscriminate plough through a side area LOSES points.  The
# camera says which colour is where; this pushes ONE named cylinder.
PUSH_STANDOFF = 165.0        # nose-to-axle plus room to line up behind it
PUSH_OVERRUN  = 55.0         # drive on past the boundary so it is properly in


def push_to_zone(rb, cx, cy, box, log=lambda *a, **k: None, clk=None):
    """Take the cylinder at (cx, cy) into the rectangle `box` with the plough.

    Approach from the far side of the cylinder along the line to the zone
    centre, then drive straight through.  No docking, no tolerance to speak
    of: the zones are 200 mm across and the piece only has to be inside one.
    """
    # AIM AT THE NEAREST PART OF THE ZONE, NOT ITS CENTRE.  The zones are 200 mm
    # across and a patient only has to be inside one, so aiming at the middle
    # buys nothing and costs everything: from a sticker at x 80 the centre of
    # PCC_L is a diagonal, which puts the stand-off point outside the west wall
    # and the robot cannot get behind the piece at all (0 of 6, measured).  The
    # nearest point makes the same push due north, and it is shorter.
    m = 40.0
    zx = float(np.clip(cx, box[0] + m, box[2] - m))
    zy = float(np.clip(cy, box[1] + m, box[3] - m))
    # AND THE ROBOT HAS TO FIT ON THE PUSH LINE, not just at its start.  Aiming
    # a due-north push at x 80 sends a 235 mm-wide robot up a corridor 80 mm
    # from the west wall: it grinds the wall and the cylinder never moves (0 of
    # 6, 25 s each, measured).  Clamp the AIM into the reachable box too and let
    # the plough's width cover the offset -- the zone is 200 mm across, so the
    # piece still lands inside it.
    _half = AgentA.W/2.0 + 12.0
    zx = float(np.clip(zx, _half, Field.W - _half))
    zy = float(np.clip(zy, _half, Field.H - _half))
    dx, dy = zx - cx, zy - cy
    d = np.hypot(dx, dy)
    if d < 1e-6:
        return
    ux, uy = dx/d, dy/d
    # stage behind the cylinder, on the line to the zone
    sx, sy = cx - ux*PUSH_STANDOFF, cy - uy*PUSH_STANDOFF
    # ...BUT THE ROBOT IS 235 WIDE AND THE FIELD HAS WALLS.  Six of six pushes
    # to PCC_L failed because the staging point was at x 80, which is inside
    # the west wall -- the robot cannot stand there, so it never went, and the
    # cylinder never moved.  Clamp the stand-off into the reachable box; the
    # plough is 130 mm across plus wings, so it still catches a cylinder half a
    # bar off the centreline and the zones are 200 mm wide.
    half = AgentA.W/2.0 + 12.0
    sx = float(np.clip(sx, half, Field.W - half))
    sy = float(np.clip(sy, half, Field.H - half))
    # THE HEADING STAYS ON THE ZONE.  Re-aiming it at the cylinder after the
    # clamp is what turned six good pushes into three: the robot then drove a
    # line that did not end in the zone and shoved the piece into the wall.
    heading = np.degrees(np.arctan2(zy - sy, zx - sx))
    # how far the cylinder sits off that line -- the plough is a bar 130 wide
    # with 45 mm wings, so it catches about 100 mm either side, and beyond that
    # the push would miss and the piece is better left where it is (-3) than
    # shoved somewhere wrong (-5).
    hx, hy = np.cos(np.radians(heading)), np.sin(np.radians(heading))
    off = abs(-(cx - sx)*hy + (cy - sy)*hx)
    reach = AgentA.TRIP_W/2.0 + AgentA.PLOUGH_WING*np.cos(
        np.radians(AgentA.PLOUGH_WING_DEG)) - Piece.CYL_D/2.0
    if off > reach:
        log("      skipped (%.0f,%.0f): %.0f mm off the push line, plough reaches %.0f"
            % (cx, cy, off, reach))
        return
    t0 = clk() if clk else 0.0
    yield from guard(pursue(rb, sx, sy, speed=220.0, tol=30.0), 14.0)
    yield from guard(turn_to(rb, heading, tol=4.0), 8.0)
    # far enough that the cylinder is past the near edge and properly inside:
    # measured from where the robot ACTUALLY stands, not from the cylinder.
    run = float(np.hypot(zx - sx, zy - sy)) + PUSH_OVERRUN
    yield from guard(drive_straight(rb, run, speed=200.0), run/120.0 + 6.0)
    if clk:
        log("      pushed (%.0f,%.0f) %.0f mm into the zone in %.1f s"
            % (cx, cy, d, clk() - t0))
