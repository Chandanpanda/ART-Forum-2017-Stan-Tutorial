...
a Raspberry Pi Camera Module 2 at each end:
    ...
    range error         0.54 m      at 100 m, 0.54% of the range
    yaw budget          0.65        of 0.005 degrees between the faces
    first mode           281 Hz     against the 200 the brief asks
    ring bore          +0.37 mm     the winding head enters every joint
    cell                21.9 min    30 joints; X 1399 of 1400, Y 130 of 150
...
at 51.2 g and 0.66 m, so the whole span of the feasible set is 1.4 g and
...
camera stands 21.1 mm off the chord ends, solved so that nothing of the
truss is inside a 62.2 x 48.8 degree field (truss.mount.fov_standoff), and
the six struts of each nose bond to the module's own lens housing so its
